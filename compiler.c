...
bool compile(const char* source, Chunk* chunk) {
  ...
  // To solve this:
  // We add a flag to track whether we’re currently in panic mode. (@SEE_VARIBALE :: panice_mode)
  // When an error occurs, we set it. (@SEE_FUNCTION :: error_at)

  // After that, we go ahead and keep compiling as normal as if the error never occurred.
  ...
  // Panic mode ends when the parser reaches a synchronization point. (where the flags get reset)
